...
import net.ethobat.system0.api.network.physical.ITransceiverAntenna;
...
public abstract class AbstractBETransceiverDeprecated extends AbstractBEAntennaDeprecated implements ITransceiverAntenna {

    private final Set<INetConnection> transmitters;
    private final Set<INetConnection> receivers;

    public <T extends S0Block> AbstractBETransceiverDeprecated(BlockEntityType<?> type, BlockPos pos, BlockState state) {
    ...
    public Set<INetConnection> getConnections() {
        //Set <INetConnection> union = Set.copyOf(this.transmitters);
        Set<INetConnection> union = new HashSet<INetConnection>(this.transmitters);
        union.addAll(this.receivers);
        return union;
    }
